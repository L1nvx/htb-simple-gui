...
def activate_machine(machine_id):
    ...
    response = session.post(
        activate_machine_url, headers=headers, json=payload, timeout=20
    )
...
class HTBGUI:
    ...
    def setup_style(self):
        ...
        self.style.configure(
            "TLabelframe.Label", background=bg_color, foreground=accent_color
        )
    ...
    def setup_left_panel(self, parent):
        ...
        self.machine_list = ttk.Combobox(
            machine_frame, state="readonly", font=("Iosevka Nerd Font", 9)
        )
        ...
        for col, (text, command) in enumerate(control_buttons):
            ttk.Button(btn_grid, text=text, style="TButton", command=command).grid(
        ...
        rows = [
            (
                "Category:",
                ttk.Combobox(
                    payload_frame, state="readonly", font=("Iosevka Nerd Font", 9)
                ),
            ),
            (
                "Type:",
        ...
        # Preview del Payload
        ttk.Label(
            payload_frame, text="Preview:", font=("Iosevka Nerd Font", 9, "bold")
        ...
        self.payload_text.grid(
            row=6, column=0, columnspan=2, sticky="nsew", padx=5, pady=(0, 5)
        )
...
"""}
    ...
    def setup_right_panel(self, parent):
        ...
        ttk.Label(
            header, text="Current Status", font=("Iosevka Nerd Font", 9, "bold")
        ).pack(side=tk.LEFT)
        ttk.Button(header, text="⟳ Refresh", command=self.check_status, width=10).pack(
            side=tk.RIGHT
        )
        ...
        self.status_labels = {}
        self.avatar_label = None  # Añadir esta línea

        for row, (label, key) in enumerate(fields):
            ttk.Label(
                grid_frame, text=label, font=("Iosevka Nerd Font", 9, "bold")
        ...
        ttk.Button(flag_frame, text="🚩 Submit", command=self.submit_flag).pack(
    ...
    def setup_console(self):
        ...
        scrollbar = ttk.Scrollbar(
            activity_frame, orient=tk.VERTICAL, command=self.activity_tree.yview
        )
    ...
    def fetch_activity(self):
        ...
        if not selected:
            ...
            self.root.after(
                0, lambda: self.activity_tree.delete(
    ...
    def update_activity_tree(self, data):
        ...
        for entry in data:
            ...
            if entry.get("is_root_blood", False):
                blood_status += "🩸"
            user_time = (
                entry.get("user_own_time", "0H 00M 00S")
                .replace("H", "H ")
                .replace("M", "M ")
            )
            root_time = (
                entry.get("root_own_time", "0H 00M 00S")
                .replace("H", "H ")
                .replace("M", "M ")
            )
    ...
    def check_status(self):
        try:
            ...
            if status:
                ...
                if avatar_url:
                    avatar_url = urljoin(
                        "https://labs.hackthebox.com", avatar_url)
                    threading.Thread(
                        target=self.load_avatar, args=(
                            avatar_url,), daemon=True
                    ).start()
                expires = status.get("expires_at", "N/A")
                if expires and expires != "N/A":
                    try:
    ...
    def load_avatar(self, url):
        try:
            ...
            if response.status_code == 200:
                ...
                self.root.after(0, self.update_avatar, photo)
        except Exception as e:
            self.log_to_console(f"Error loading avatar: {str(e)}", "error")
